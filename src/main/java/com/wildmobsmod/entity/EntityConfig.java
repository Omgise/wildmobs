...
public class EntityConfig {
    ...
    /**
     * Factory method: Constructs a new EntityConfig with the given default settings and registers it to the
     * {@link EntityConfig#syncAll(Configuration) syncAll()} system.
     */
    ...
    protected EntityConfig(String id, int defaultSpawnrate, int defaultMinPack, int defaultMaxPack,
    ...
    protected void sync(Configuration config) {
        enabled = config.getBoolean(
            ...
            defaultEnabled,
            String.format(
                "Register the %s Entity to the Game Registry? (Setting this to false will completely disable it.)",
                id));
        spawnrate = config.getInt(
            "spawnrate",
            getConfigCategory(),
            defaultSpawnrate,
            0,
            100,
            String.format(
                "The spawnrate of the %s Entity - higher means more frequent spawns. Set to 0 to disable natural spawning (it will still be registered).",
                id));
        minPack = config.getInt(
            "minPackSize",
            getConfigCategory(),
            defaultMinPack,
            1,
            20,
            String.format(
                "The minimum number of %s Entities to spawn per pack - has to be lower than maxPackSize (if it's larger, all packs will spawn with a size of minPackSize).",
                id));
        maxPack = Math.max(
            minPack,
            config.getInt(
                ...
                20,
                String.format(
                    "The maximum number of %s Entities to spawn per pack - has to be larger than minPackSize (if it's lower, all packs will spawn with a size of minPackSize).",
                    id)));
    }
    ...
    protected static WeightedRandomSelector<ItemStack> buildWeightedRandomStackSelector(String... stackList) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            try {
                ...
                pos = def.lastIndexOf(',');
                int stacksize = Integer.parseInt(
                    def.substring(pos + 1)
                        .trim());
                def = def.substring(0, pos)
                    .trim();
                ...
                FMLLog
                    .log(WildMobsMod.MODID, Level.ERROR, e, "Invalid config entry: \"%s\" (index:%d)", stackList[i], i);
                stack = null;
                weight = 0;
            }
            stacks[i] = stack;
        ...
        for (int i = 0; i < size; i++) {
            ...
            try {
                ...
                FMLLog.log(
                    ...
                    Level.ERROR,
                    e,
                    "Invalid config entry: \"%s\" (index:%d)",
                    stackArray[i],
                    i);
                stack = null;
            }
