...
public class EntityAIAttackIfApproached extends EntityAITarget {
    ...
    public EntityAIAttackIfApproached(EntityCreature creature, Class targetClass, int targetChance, boolean checkLOS,
        boolean nearbyOnly, final IEntitySelector selector) {
            ...
            public boolean isEntityApplicable(Entity e) {
                return !(e instanceof EntityLivingBase) ? false
                    : (selector != null && !selector.isEntityApplicable(e) ? false
                        : EntityAIAttackIfApproached.this.isSuitableTarget((EntityLivingBase) e, false));
            }
        };
    }
    ...
    public boolean shouldExecute() {
        if (this.targetChance > 0 && this.taskOwner.getRNG()
            .nextInt(this.targetChance) != 0) return false;
        double d0 = this.getTargetDistance();
        List list = this.taskOwner.worldObj.selectEntitiesWithinAABB(
            this.targetClass,
            this.taskOwner.boundingBox.expand(2.0D, 4.0D, 2.0D),
