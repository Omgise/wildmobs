    ...
    public void startExecuting() {
        ...
        AxisAlignedBB aabb = AxisAlignedBB
            .getBoundingBox(
                ...
                this.creature.posY,
                this.creature.posZ,
                this.creature.posX + 1.0D,
                this.creature.posY + 1.0D,
                this.creature.posZ + 1.0D)
            .expand(8.0D, 8.0D, 8.0D);
        List list = this.creature.worldObj.getEntitiesWithinAABB(this.creature.getClass(), aabb);
        Iterator iterator = list.iterator();
        while (iterator.hasNext()) {
            EntityCreature entitycreature = (EntityCreature) iterator.next();
            if (this.creature != entitycreature && entitycreature.getAttackTarget() == null
                && ((EntityAnimal) entitycreature).getAge() >= 0
                && !entitycreature.isOnSameTeam(this.creature.getAITarget())
                && this.creature.worldObj.rand.nextInt(2) == 0) {
