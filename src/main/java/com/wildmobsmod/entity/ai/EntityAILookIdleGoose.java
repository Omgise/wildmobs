...
public class EntityAILookIdleGoose extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        if (this.idleEntity.onGround == false || ((EntityGoose) this.idleEntity).getFlyingState() == 1
            || ((EntityGoose) this.idleEntity).getFlyingState() == 2) {
            return false;
        } else {
            return this.idleEntity.getRNG()
                .nextFloat() < 0.02F;
        }
    }
    ...
    public void startExecuting() {
        double d0 = (Math.PI * 2D) * this.idleEntity.getRNG()
            .nextDouble();
        this.lookX = Math.cos(d0);
        this.lookZ = Math.sin(d0);
        this.idleTime = 20 + this.idleEntity.getRNG()
            .nextInt(20);
    }
