...
public class EntityAIMeatTempt extends EntityAIBase {
    ...
    public boolean continueExecuting() {
        if (this.scaredByPlayerMovement) {
            if (this.temptedEntity.getDistanceSqToEntity(this.temptingPlayer) < 36.0D) {
                ...
                if (Math.abs((double) this.temptingPlayer.rotationPitch - this.temptingRotPitch) > 5.0D
                    || Math.abs((double) this.temptingPlayer.rotationYaw - this.temptingRotYaw) > 5.0D) {
                    return false;
                }
            } else {
    ...
    public void startExecuting() {
        ...
        this.targetZ = this.temptingPlayer.posZ;
        this.isRunning = true;
        this.avoidWater = this.temptedEntity.getNavigator()
            .getAvoidsWater();
    ...
    public void resetTask() {
        ...
        this.temptedEntity.getNavigator()
            .setAvoidsWater(this.avoidWater);
    }
    ...
    public void updateTask() {
        this.temptedEntity.getLookHelper()
            .setLookPositionWithEntity(this.temptingPlayer, 30.0F, (float) this.temptedEntity.getVerticalFaceSpeed());
        ...
        if (this.temptedEntity.getDistanceSqToEntity(this.temptingPlayer) < 6.25D) {
            ...
            this.temptedEntity.getNavigator()
                .tryMoveToEntityLiving(this.temptingPlayer, this.speedTowardsTarget);
        }
    }
