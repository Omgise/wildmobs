...
public class EntityAIMoveToCrops extends EntityAIBase {
    ...
    private Vec3 findPossibleShelter() {
        ...
        for (int i = 0; i < 10; ++i) {
            int j = MathHelper
                .floor_double(this.theCreature.posX + (double) random.nextInt(20) - (double) random.nextInt(20));
            int k = MathHelper.floor_double(
                this.theCreature.boundingBox.minY + (double) random.nextInt(6) - (double) random.nextInt(6));
            int l = MathHelper
                .floor_double(this.theCreature.posZ + (double) random.nextInt(20) - (double) random.nextInt(20));
