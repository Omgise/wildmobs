...
public class EntityAIMoveToWater extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        if (this.theCreature.onGround) {
            ...
            return this.theCreature.getRNG()
                .nextFloat() < 0.02F;
        }
        return false;
    }
    ...
    private Vec3 findPossibleShelter() {
        ...
        for (int i = 0; i < 20; ++i) {
            int j = MathHelper
                .floor_double(this.theCreature.posX + (double) random.nextInt(40) - (double) random.nextInt(40));
            int k = MathHelper.floor_double(this.theCreature.boundingBox.minY + (double) random.nextInt(12) - 3.0D);
            int l = MathHelper
                .floor_double(this.theCreature.posZ + (double) random.nextInt(40) - (double) random.nextInt(40));
            if (this.theWorld.getBlock(j, k - 1, l) == Blocks.water)
