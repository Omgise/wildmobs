...
public class EntityAIWMOcelotSit extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        return this.theOcelot.isTamed() && !this.theOcelot.isSitting()
            && this.theOcelot.getRNG()
                .nextDouble() <= 0.006500000134110451D
            && this.findTargetCoords();
    }
    ...
    public boolean continueExecuting() {
        return this.boredom <= this.initiative && this.sitTime <= 60
            && this.willSitOn(this.theOcelot.worldObj, this.targetX, this.targetY, this.targetZ);
    }
    ...
    public void startExecuting() {
        this.theOcelot.getNavigator()
            .tryMoveToXYZ(this.targetX + 0.5D, this.targetY + 1, this.targetZ + 0.5D, this.speed);
        this.boredom = 0;
        this.sitTime = 0;
        this.initiative = this.theOcelot.getRNG()
            .nextInt(
                this.theOcelot.getRNG()
                    .nextInt(1200) + 1200)
            + 1200;
        this.theOcelot.func_70907_r()
            .setSitting(false);
    }
    ...
    public void updateTask() {
        ...
        this.theOcelot.func_70907_r()
            .setSitting(false);
        if (this.theOcelot.getDistanceSq((double) this.targetX, (double) (this.targetY + 1), (double) this.targetZ)
            > 1.0D) {
            this.theOcelot.setSitting(false);
            this.theOcelot.getNavigator()
                .tryMoveToXYZ(
                    (double) ((float) this.targetX) + 0.5D,
                    (double) (this.targetY + 1),
                    (double) ((float) this.targetZ) + 0.5D,
                    this.speed);
            ++this.sitTime;
    ...
    private boolean findTargetCoords() {
        ...
        for (int j = (int) this.theOcelot.posX - 8; (double) j < this.theOcelot.posX + 8.0D; ++j) {
            for (int k = (int) this.theOcelot.posZ - 8; (double) k < this.theOcelot.posZ + 8.0D; ++k) {
                if (this.willSitOn(this.theOcelot.worldObj, j, i, k)
                    && this.theOcelot.worldObj.isAirBlock(j, i + 1, k)) {
