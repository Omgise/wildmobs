...
public class EntityConfigDreathMired extends EntityConfig {
    ...
    /**
     * Adaptation of the {@link EntityConfig#construct(String, int, int, int, boolean) EntityConfig.construct()} factory
     * method.
     */
    public static EntityConfigDreathMired construct(String id, int spawnrate, int minPack, int maxPack, boolean enabled,
        boolean enableBabyDreath, boolean registerMiredEntities, double bottleDropChance, boolean enableMiredBottle,
        int miredDropCount, String[] miredLoot, boolean guaranteeDropCount) {
        EntityConfigDreathMired cfg = new EntityConfigDreathMired(
            id,
            spawnrate,
            minPack,
            maxPack,
    ...
    protected final boolean defaultEnableBabyDreath, defaultRegisterMiredEntities, defaultEnableMiredBottle,
        defaultGuaranteeDropCount;
    ...
    protected EntityConfigDreathMired(String id, int defaultSpawnrate, int defaultMinPack, int defaultMaxPack,
        boolean defaultEnabled, boolean defaultEnableBabyDreath, boolean defaultRegisterMiredEntities,
        double defaultBottleDropChance, boolean defaultEnableMiredBottle, int defaultMiredDropCount,
        String[] defaultMiredLoot, boolean defaultGuaranteeDropCount) {
    ...
    protected void sync(Configuration config) {
        super.sync(config);
        enableBabyDreath = config
            .getBoolean("enableBabyDreath", getConfigCategory(), defaultEnableBabyDreath, "Allow Baby Dreaths?");
        registerMiredEntities = config.getBoolean(
            "registerMiredEntities",
            getMiredCategory(),
            defaultRegisterMiredEntities,
            "Register the Mired and MiredSummoner Entities?");
        bottleDropChance = config
            .get(
                getMiredCategory(),
                "bottleDropChance",
                defaultBottleDropChance,
                "Chance (in percent) of a mired dropping a mired bottle.",
                0.0D,
                1.0D)
            .getDouble();
        enableMiredBottle = config.getBoolean(
            "enableMiredBottle",
            getMiredCategory(),
            defaultEnableMiredBottle,
            "Allow MiredSummoners (and consequentially Mireds) to be spawned via Mired Bottles (which are dropped by Dreaths)? [Ignored if \"registerMiredEntities\" is false]");
        miredDropCount = config.getInt(
            "miredDropCount",
            getMiredCategory(),
            defaultMiredDropCount,
            1,
            20,
            "Amount of Loot a Mired Bottle is meant to drop.");
        miredLoot = config.getStringList(
            "miredLoot",
            getMiredCategory(),
            defaultMiredLoot,
            "Mired LootTable definition. " + PATTERN_WEIGHTEDRANDOMSTACKSELECTOR);
        miredLootTable = buildWeightedRandomStackSelector(miredLoot);
        guaranteeDropCount = config.getBoolean(
            "guaranteeDropCount",
            getMiredCategory(),
            defaultGuaranteeDropCount,
            "If a MiredSummoners loot pool hasn't been exhausted during its lifetime, drop the leftovers on it's own death?");
    }
