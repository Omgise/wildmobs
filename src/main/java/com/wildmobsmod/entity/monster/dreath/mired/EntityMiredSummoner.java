...
public class EntityMiredSummoner extends Entity {
    ...
    private static final int DROPPING_THRESHOLD = EXPECTED_LIFETIME - 60, DROPPING_DURATION = DROPPING_THRESHOLD - 100,
        AVERAGE_DROPPING_TRIES = 10;
                ...
                if (WildMobsMod.DREATH_MIRED_CONFIG.getGuaranteeDropCount()) {
                    while (remainingDrops-- > 0) {
                        spawnMired(
                            ...
                            (int) posY,
                            (int) posZ,
                            WildMobsMod.DREATH_MIRED_CONFIG.getRandomLootStack());
                    }
                }
                this.setDead();
            } else {
    ...
    private boolean trySpawnMired() {
        ...
        if ((block.isAir(worldObj, miredX, miredY, miredZ)
            || block.getCollisionBoundingBoxFromPool(worldObj, miredX, miredY, miredZ) == null)
            && worldObj.getBlock(miredX, miredY - 1, miredZ)
                .getMaterial()
                .blocksMovement()) {
