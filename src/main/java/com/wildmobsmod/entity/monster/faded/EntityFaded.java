...
public class EntityFaded extends EntityZombie {
    ...
    private static final AttributeModifier backPackSlonwessModifier = new AttributeModifier(
        backPackSlownessUUID,
        "Back pack slowness",
        -0.25D,
        1);
    ...
    protected void dropEquipment(boolean playerkill, int looting) {
        ...
        if (itemstack != null && (playerkill || flag1)
            && this.rand.nextFloat() - (float) looting * 0.01F < this.getHandEquipmentDropChance()) {
    ...
    public void onLivingUpdate() {
        ...
        if (!this.worldObj.isRemote && this.canPickUpLoot()
            ...
            List list = this.worldObj
                .getEntitiesWithinAABB(EntityItem.class, this.boundingBox.expand(1.0D, 0.0D, 1.0D));
                ...
                if (!entityitem.isDead && entityitem.getEntityItem() != null) {
                    ...
                    if (i > -1) {
                        ...
                        if (!flag) {
                            ...
                            if (itemstack2 != null) {
                                if ((this.isItemUseful(itemstack) && !(this.isItemUseful(itemstack2)))
                                    || (this.isItemEdible(itemstack) && !(this.isItemEdible(itemstack2)))) {
                                    flag1 = true;
                                } else {
                                    flag1 = false;
                                }
                            ...
                            if (flag1 == true) {
                                if (itemstack.getItem() == Items.diamond && entityitem.func_145800_j() != null) {
                                    EntityPlayer entityplayer = this.worldObj
                                        .getPlayerEntityByName(entityitem.func_145800_j());
        ...
        if (!this.worldObj.isRemote) {
            ...
            if (this.getAttackTarget() != null && itemstack != null
                && (itemstack.getItemUseAction() == EnumAction.drink || itemstack.getItemUseAction() == EnumAction.eat)
                && this.eating <= 0
                && this.rand.nextFloat() < 0.2F) {
                if (itemstack.getItem() instanceof ItemFood) {
                    if (this.getHealth() < this.getMaxHealth()
                        - ((float) ((ItemFood) itemstack.getItem()).func_150905_g(itemstack) + 2.0F)) {
                        this.eating = 26;
                    }
                } else {
                    this.eating = 26;
                }
            } else if (this.getAttackTarget() != null && itemstack != null
                && itemstack.getItemUseAction() != EnumAction.drink
                && itemstack.getItemUseAction() != EnumAction.eat
                && itemstack2 != null
                && this.rand.nextFloat() < 0.1F) {
                    if ((this.isItemUseful(itemstack2) && !(this.isItemUseful(itemstack)))
                        || (this.isItemEdible(itemstack2) && !(this.isItemEdible(itemstack)))) {
                        if (itemstack2.getItem() instanceof ItemFood) {
                            if (this.getHealth() < this.getMaxHealth()
                                - ((float) ((ItemFood) itemstack2.getItem()).func_150905_g(itemstack) + 2.0F)) {
            ...
            if (this.eating > 0) {
                if (this.eating == 1) {
                    ...
                    } else if (itemstack != null && itemstack.getItem() == Items.golden_apple
                    ...
                    if (itemstack != null && itemstack.getItem()
                        .getItemUseAction(itemstack) == EnumAction.eat) {
                ...
                if (this.eatingSound == 0) {
                    if (itemstack != null && itemstack.getItem()
                        .getItemUseAction(itemstack) == EnumAction.drink) {
                        this.playSound("random.drink", 0.5F, this.worldObj.rand.nextFloat() * 0.1F + 0.9F);
                    } else if (itemstack != null && itemstack.getItem()
                        .getItemUseAction(itemstack) == EnumAction.eat) {
                            this.playSound(
                                "random.eat",
        ...
        if (!this.worldObj.isRemote && this.rand.nextInt(20) == 0) {
            ...
            List list2 = this.worldObj
                .getEntitiesWithinAABB(this.getClass(), this.boundingBox.expand(8.0D, 4.0D, 8.0D));
            list2.clear();
            int i = 0;
    ...
    public void spawnPack() {
        ...
        for (int k = 0; k < 3 + this.rand.nextInt(2); k++) {
            if (suitableblocks.size() > 0) {
                ChunkCoordinates spawnblock = (ChunkCoordinates) suitableblocks
                    .get(this.rand.nextInt(suitableblocks.size()));
    ...
    public boolean getCanSpawnHere() {
        if ((this.worldObj.getWorldInfo()
            .isRaining()
            || this.worldObj.getWorldInfo()
                .isThundering())
            && this.worldObj.provider instanceof WorldProviderSurface
