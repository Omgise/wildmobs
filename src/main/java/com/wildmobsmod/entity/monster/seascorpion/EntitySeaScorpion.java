...
public class EntitySeaScorpion extends EntityMobTameable {
    ...
    private boolean canSpawnPack = true; // prevent pack members from spawning their own packs - only used for on-spawn
                                         // handling
    ...
    public final IEntitySelector targetEntitySelectorMob = new IEntitySelector() {
        ...
        public boolean isEntityApplicable(Entity entity) {
            return entity.isEntityAlive() && EntitySeaScorpion.this.getEntitySenses()
                .canSee(entity)
                && EntitySeaScorpion.this.worldObj
                    .getBlock(
                        MathHelper.floor_double(entity.posX),
                        MathHelper.floor_double(entity.posY),
                        MathHelper.floor_double(entity.posZ))
                    .getMaterial() == Material.water
                && entity != EntitySeaScorpion.this
                && !(entity instanceof EntitySeaScorpion)
                && !(entity instanceof EntityCreeper && !(entity instanceof EntityGhast)
                    && ((EntityMob) entity).getActivePotionEffect(WildMobsMod.potionAquaHealing) == null);
        }
    };
        ...
        public boolean isEntityApplicable(Entity entity) {
            return entity.isEntityAlive() && EntitySeaScorpion.this.getEntitySenses()
                ...
                && (!((EntityPlayer) entity).capabilities.disableDamage
                    && ((EntityPlayer) entity).getActivePotionEffect(WildMobsMod.potionAquaHealing) == null);
        }
    };
    ...
    public void setSize(int size) {
        ...
        this.getEntityAttribute(SharedMonsterAttributes.maxHealth)
            .setBaseValue((double) (8 + 4 * size));
        this.experienceValue = 3 + size;
        this.setHealth(this.getMaxHealth());
        this.getEntityAttribute(SharedMonsterAttributes.attackDamage)
            .setBaseValue(this.getSize());
    }
    ...
    protected String getLivingSound() {
        return this.worldObj
            .getBlock(
                ...
                MathHelper.floor_double(this.posY),
                MathHelper.floor_double(this.posZ))
            .getMaterial() == Material.water ? "wildmobsmod:mob.seascorpion.saywater" : null;
    }
    ...
    protected String getHurtSound() {
        return this.worldObj
            .getBlock(
                ...
                MathHelper.floor_double(this.posY),
                MathHelper.floor_double(this.posZ))
            .getMaterial() == Material.water ? "wildmobsmod:mob.seascorpion.hurtwater"
                : "wildmobsmod:mob.seascorpion.hurtland";
    }
    ...
    public void onLivingUpdate() {
        ...
        checkBlockCollision();
        final int flooredX = MathHelper.floor_double(posX), flooredY = MathHelper.floor_double(posY),
            flooredZ = MathHelper.floor_double(posZ);
        ...
        if (blockAtPos.getMaterial() == Material.water) {
            if (timeToJump > 0) {
                ...
                if (cachedPosition != null) {
                    ...
                    motionY *= entityToAttack == null
                        ? cachedPosition.posY - 0.5D > posY || cachedPosition.posY + 0.5D < posY ? 0.6D : 0.4D
                        : 0.8D;
                } else {
                    motionY = 0.0D;
                }
            ...
            if (entityToAttack != null) {
                Block block = worldObj.getBlock(
                    ...
                    MathHelper.floor_double(entityToAttack.posY + entityToAttack.getEyeHeight()),
                    MathHelper.floor_double(entityToAttack.posZ));
                if (!block.isNormalCube() && block.getMaterial() != Material.water
                    && rand.nextInt(30) == 0
                    && getDistanceSqToEntity(entityToAttack) < 16.0D) {
                    timeToJump = 20;
                }
            }
        }

        if (timeToJump > 0 && !blockAtPos.isNormalCube()
            && blockAtPos.getMaterial() != Material.water
            && worldObj.getBlock(flooredX, flooredY - 1, flooredZ)
                .getMaterial() == Material.water) {
            timeToJump = 0;
            waterJump();
        }

        if ((cachedPosition == null || rand.nextInt(100) == 0
            || cachedPosition.getDistanceSquared(flooredX, flooredY, flooredZ) < 2.0F
            || cachedPosition.getDistanceSquared(flooredX, (int) cachedPosition.posY, flooredZ) < 1.0F)
            && entityToAttack == null) {
            cachedPosition = new ChunkCoordinates(
                flooredX + rand.nextInt(12) - rand.nextInt(12),
                flooredY + rand.nextInt(8) - 2,
                flooredZ + rand.nextInt(12) - rand.nextInt(12));
        } else if (entityToAttack != null) {
            cachedPosition = null;
            final int targetX = (int) entityToAttack.posX, targetY = (int) entityToAttack.posY,
                targetZ = (int) entityToAttack.posZ;
            if (targetY < (int) (targetY + entityToAttack.getEyeHeight())
                && worldObj.getBlock(targetX, (int) (targetY + entityToAttack.getEyeHeight()), targetZ)
                    .getMaterial() == Material.water) {
        ...
        if (cachedPosition != null && worldObj.getBlock(cachedX, cachedY, cachedZ)
            .getMaterial() != Material.water || cachedY < 1) {
            cachedPosition = null;
        }
        ...
        if (getGrowingAge() >= 0) {
            if (getIsSeaMonster() || (hasCustomNameTag() && "Leviathan".equals(getCustomNameTag()))) {
                ...
                findMobToAttack();
            } else if (!canSeaScorpionSeeSky() && !worldObj.isDaytime()
                && worldObj.isThundering()
                && worldObj.isRaining()
                && getBrightness(1.0F) < 0.5F) {
                    findHumanToAttack();
                }
        }
        ...
        if (entityToAttack != null) {
            if (getGrowingAge() < 0 || !canAttackTargetBeSeen(entityToAttack)) {
                ...
                final int targetX = MathHelper.floor_double(entityToAttack.posX),
                    targetY = MathHelper.floor_double(entityToAttack.posY),
                    targetZ = MathHelper.floor_double(entityToAttack.posZ);
                if (worldObj.getBlock(targetX, targetY, targetZ)
                    .getMaterial() != Material.water
                    && worldObj.getBlock(targetX, targetY - 1, targetZ)
                        .getMaterial() != Material.water) {
                    entityToAttack = null;
                }
            }
        ...
        if (regen < 0) {
            ...
            if (!getIsWild() && getHealth() < getEntityAttribute(SharedMonsterAttributes.maxHealth).getAttributeValue()
                && blockAtPos.getMaterial() == Material.water) {
        ...
        if (worldObj.isRemote && eatingTimer > 0) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                worldObj.spawnParticle(
                    particleName,
                    posX + (double) (rand.nextFloat() * width * 1.5F) - (double) width,
                    posY + 0.5D + (double) (rand.nextFloat() * height) - 0.5D,
                    posZ + (double) (rand.nextFloat() * width * 1.5F) - (double) width,
                    d0,
                    d1,
                    d2);
            }
    ...
    protected void checkBlockCollision() {
        ...
        if (spawnZ > posZ && worldObj.getBlock(x, y, z + 1)
            .isNormalCube()) {
            cachedPosition = entityToAttack == null
                ? new ChunkCoordinates(
                    x + rand.nextInt(12) - rand.nextInt(12),
                    y + rand.nextInt(8) - 2,
                    z - rand.nextInt(12))
                : null;
        } else if (spawnX < posX && worldObj.getBlock(x - 1, y, z)
            .isNormalCube()) {
                cachedPosition = entityToAttack == null
                    ? new ChunkCoordinates(
                        x + rand.nextInt(12),
                        y + rand.nextInt(8) - 2,
                        z + rand.nextInt(12) - rand.nextInt(12))
                    : null;
            } else if (spawnZ < posZ && worldObj.getBlock(x, y, z - 1)
                .isNormalCube()) {
                    cachedPosition = entityToAttack == null
                        ? new ChunkCoordinates(
                            x + rand.nextInt(12) - rand.nextInt(12),
    ...
    public boolean canAttackTargetBeSeen(Entity entity) {
        return this.worldObj.rayTraceBlocks(
            Vec3.createVectorHelper(posX, posY + (double) getEyeHeight(), posZ),
            Vec3.createVectorHelper(entity.posX, entity.posY + 0.1, entity.posZ)) == null;
    }

    public boolean attackEntityAsMob(Entity entity) {
        if (getGrowingAge() >= 0 && worldObj
            .getBlock(MathHelper.floor_double(posX), MathHelper.floor_double(posY), MathHelper.floor_double(posZ))
            .getMaterial() == Material.water) {
            ...
            if (entity.attackEntityFrom(DamageSource.causeMobDamage(this), damage + 5)) {
                if (knockback > 0) {
                    entity.addVelocity(
                        (double) (-MathHelper.sin(rotationYaw * (float) Math.PI / 180.0F) * (float) knockback * 0.5F),
                        0.1D,
                        (double) (MathHelper.cos(rotationYaw * (float) Math.PI / 180.0F) * (float) knockback * 0.5F));
                    motionX *= 0.6D;
                    motionZ *= 0.6D;
                }
    ...
    protected void findAttackTarget(IEntitySelector selector) {
        List list = worldObj
            .selectEntitiesWithinAABB(EntityLivingBase.class, boundingBox.expand(16.0D, 16.0D, 16.0D), selector);
    ...
    public void setScaleForAge(boolean isChild, int size) {
        int i = size + 1;
        this.setSize(
            (isChild ? (0.2F + (i * 0.2F)) / 2 : 0.2F + (i * 0.2F)),
            (isChild ? (0.025F + (i * 0.025F)) / 2 : 0.05F + (i * 0.05F)));
    }
    ...
    public boolean interact(EntityPlayer player) {
        ...
        if (!worldObj.isRemote && stack != null) {
            ...
            if (item == Items.water_bucket && !getIsWild() && !getIsSeaMonster()) {
                ...
                if (isHealingItem(stack) == true
                    && getHealth() < getEntityAttribute(SharedMonsterAttributes.maxHealth).getAttributeValue()
                    && !getIsWild()) {
                    if (!player.capabilities.isCreativeMode) {
                        stack.stackSize--;
                    }
    ...
    public boolean getCanSpawnHere() {
        ...
        boolean flag = worldObj.difficultySetting != EnumDifficulty.PEACEFUL && isValidLightLevel()
