...
public class EntityTarantula extends EntitySpider {
    ...
    public void onLivingUpdate() {
        ...
        if (this.entityToAttack != null) {
            double d0 = this.getDistanceSq(
                this.entityToAttack.posX,
                this.entityToAttack.boundingBox.minY,
                this.entityToAttack.posZ);
            if (this.rand.nextInt(40) == 0 && d0 < 225.0D
                && d0 > 16.0D
                && this.getEntitySenses()
                    .canSee(this.entityToAttack)
                && this.hairAttackTimer <= 0) {
                this.hairAttackTimer = 140;
            }

            if (this.hairAttackTimer > 0 && this.getEntitySenses()
                .canSee(this.entityToAttack)) {
                if (this.hairAttackTimer == 130 || this.hairAttackTimer == 127
                    || this.hairAttackTimer == 124
                    || this.hairAttackTimer == 121
                    || this.hairAttackTimer == 118
                    || this.hairAttackTimer == 115) {
                    float f;
        ...
        if (this.hairAttackTimer > 100) {
            if (this.entityToAttack != null && this.getEntitySenses()
                .canSee(this.entityToAttack)) {
                this.getEntityAttribute(SharedMonsterAttributes.movementSpeed)
                    .setBaseValue(0.0D);
            } else {
    ...
    public void attackEntityWithRangedAttack(EntityLivingBase target, float damage) {
        EntityTarantulaHair entitytarantulahair = new EntityTarantulaHair(
            this.worldObj,
            this,
            target,
            1.2F,
            (float) (14 - this.worldObj.difficultySetting.getDifficultyId() * 2));
        this.playSound(
            "random.bow",
            1.0F,
            1.0F / (this.getRNG()
