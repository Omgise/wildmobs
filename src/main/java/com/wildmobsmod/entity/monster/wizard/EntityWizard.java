...
public class EntityWizard extends EntityMob implements IRangedAttackMob {
    ...
    private static final Item[] wizardDrops = new Item[] { Items.speckled_melon, Items.magma_cream, Items.ghast_tear,
        Items.glass_bottle, Items.golden_apple };
    ...
    public void onLivingUpdate() {
        if (!this.worldObj.isRemote) {
            if (this.rand.nextFloat() < 0.02F && this.getHealth() < this.getMaxHealth()
                && !this.isPotionActive(Potion.regeneration)) {
                if (this.drinking <= 0) {
                    this.drinking = 26;
                    this.setCurrentItemOrArmor(0, new ItemStack(Items.potionitem, 1, 8289));
                    this.equipmentDropChances[0] = 0.085F;
                }
            } else if (this.rand.nextFloat() < 0.10F && this.isInsideOfMaterial(Material.water)
                ...
                if (this.rand.nextFloat() < 0.10F && this.isBurning() && !this.isPotionActive(Potion.fireResistance)) {
                    if (this.drinking <= 0) {
                        this.drinking = 26;
                    ...
                    && this.getHealth() < 22.0F
                    && !this.isPotionActive(Potion.field_76444_x)) {
                        if (this.drinking <= 0) {
                            this.drinking = 26;
            ...
            if (this.rand.nextFloat() < 0.005F && this.getAttackTarget() != null
                && this.getAttackTarget()
                    .getDistanceSqToEntity(this) <= 36.0D) {
                this.teleportRandomly();
            } else if (this.rand.nextFloat() < 0.008F && this.getAttackTarget() != null
                && this.getAttackTarget()
                    .getDistanceSqToEntity(this) <= 36.0D
        ...
        if (this.worldObj.blockExists(i, j, k)) {
            ...
            if (flag1) {
                ...
                if (this.worldObj.getCollidingBoundingBoxes(this, this.boundingBox)
                    .isEmpty() && !this.worldObj.isAnyLiquid(this.boundingBox)) {
                    flag = true;
                }
            }
        }
    ...
    public void attackEntityWithRangedAttack(EntityLivingBase living, float damage) {
        if (this.attackCooldown <= 0 && this.drinking <= 0) {
            ...
            this.playSound(
                "wildmobsmod:random.spell.spell",
                1.0F,
                1.0F / (this.getRNG()
                    .nextFloat() * 0.4F + 0.8F));
