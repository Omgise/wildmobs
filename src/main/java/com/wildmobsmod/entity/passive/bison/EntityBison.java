        ...
        if (this.isEntityInvulnerable()) {
            ...
            if (entity instanceof EntityPlayer && this.getGrowingAge() < 0) {
                List list = this.worldObj
                    .getEntitiesWithinAABBExcludingEntity(this, this.boundingBox.expand(6.0D, 6.0D, 6.0D));

                for (int i = 0; i < list.size(); ++i) {
                    Entity entity1 = (Entity) list.get(i);

                    if (entity1 instanceof EntityBison && ((EntityBison) entity1).getAttackTarget() == null
                        && ((EntityBison) entity1).getGrowingAge() >= 0) {
