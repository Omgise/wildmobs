...
public class EntityDireWolf extends EntityTameable {
    ...
    public float getShakeAngle(float f1, float f2) {
        float f3 = (this.prevTimeWolfIsShaking + (this.timeWolfIsShaking - this.prevTimeWolfIsShaking) * f1 + f2)
            / 1.8F;
        ...
        return MathHelper.sin(f3 * (float) Math.PI) * MathHelper.sin(f3 * (float) Math.PI * 11.0F)
            * 0.15F
            * (float) Math.PI;
    }
    ...
    public boolean attackEntityAsMob(Entity target) {
        int dmg = this.isTamed() ? 10
            : worldObj.difficultySetting == EnumDifficulty.HARD ? 7
                : worldObj.difficultySetting == EnumDifficulty.NORMAL ? 5 : 4;
            ...
            if (itemstack != null) {
                if (itemstack.getItem() instanceof ItemFood) {
                    ...
                    if (itemfood.isWolfsFavoriteMeat()
                        && this.dataWatcher.getWatchableObjectFloat(18) < getMaxHealth()) {
    ...
    public float getTailRotation() {
        return this.isAngry() ? 1.5393804F
            : (this.isTamed()
                ? (0.55F - (getMaxHealth() - this.dataWatcher.getWatchableObjectFloat(18)) * 0.02F) * (float) Math.PI
                : ((float) Math.PI / 5F));
    }
