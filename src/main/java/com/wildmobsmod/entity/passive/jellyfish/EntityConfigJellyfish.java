...
public class EntityConfigJellyfish extends EntityConfig {
     ...
     * method.
     */
    public static EntityConfigJellyfish construct(String id, int spawnrate, int minPack, int maxPack, boolean enabled,
        int netherSpawnrate) {
        EntityConfigJellyfish cfg = new EntityConfigJellyfish(
            id,
            spawnrate,
            minPack,
            maxPack,
    ...
    protected EntityConfigJellyfish(String id, int defaultSpawnrate, int defaultMinPack, int defaultMaxPack,
        boolean defaultEnabled, int defaultNetherSpawnrate) {
    ...
    protected void sync(Configuration config) {
        super.sync(config);
        netherSpawnrate = config.getInt(
            "netherSpawnrate",
            getConfigCategory(),
            defaultNetherSpawnrate,
            0,
            100,
            "Nether Medusae (nether variant of the Jellyfish) spawnrate. (Note that pack size will always be 1-2)");
    }
