...
public class EntityJellyfish extends EntityWaterMob implements ISkinnedEntity {
    ...
    private byte floatingCD; // = (byte) (Math.random() * 250D - 125); // prevent floating pattern from being
                             // synchronized (disabled, because I think it looks nice)
    ...
    public int getMaxSpawnedInChunk() {
        return worldObj.provider instanceof WorldProviderHell ? WildMobsMod.JELLYFISH_CONFIG.getNetherMaxPackSize()
            : WildMobsMod.JELLYFISH_CONFIG.getMaxPackSize();
    }
    ...
    public void onLivingUpdate() {
        ...
        if (!this.getNether()) {
            ...
            // Animations
            if (this.swimTimer > 39 && this.worldObj
                .getBlock(
                    MathHelper.floor_double(this.posX),
                    MathHelper.floor_double(this.posY),
                    MathHelper.floor_double(this.posZ))
            ...
            } else if (this.swimTimer > 0 && this.swimTimer < 20
                && this.width > 2.0F
                && this.worldObj
                    .getBlock(
                        MathHelper.floor_double(this.posX),
    ...
    public void onEntityUpdate() {
        ...
        if (this.isEntityAlive() && !this.getNether()
            && this.worldObj
                .getBlock(
                    MathHelper.floor_double(this.posX),
                    MathHelper.floor_double(this.posY),
                    MathHelper.floor_double(this.posZ))
                .getMaterial() != Material.water) // Nether Jellyfish don't "drown"
        {
            --i;
            this.setAir(i);
    ...
    public boolean getCanSpawnHere() {
        if (worldObj.provider instanceof WorldProviderHell) {
            if (super.getCanSpawnHere()) {
                ...
                // Workaround to avoid custom spawnHandler (natural spawning in liquids other than water is unsupported
                // by the Vanilla/FML frameworks); Checks if the Jellyfish will fall into a lava lake
                int lava = findLavaY(x, y, z);
                if (lava > 0) {
                    return worldObj
                        .checkNoEntityCollision(this.boundingBox.getOffsetBoundingBox(0.0D, lava - posY, 0.0D));
                }
