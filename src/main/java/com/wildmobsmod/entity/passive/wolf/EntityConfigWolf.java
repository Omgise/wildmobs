...
public class EntityConfigWolf extends EntityConfig {
     ...
     * method.
     */
    public static EntityConfigWolf construct(String id, int spawnrate, int minPack, int maxPack, boolean enabled,
        int overrideChance) {
    ...
    protected EntityConfigWolf(String id, int defaultSpawnrate, int defaultMinPack, int defaultMaxPack,
    ...
    protected void sync(Configuration config) {
        ...
        overrideChance = config.getInt(
            ...
            0,
            100,
            "Chance (in Percent) to replace a vanilla Wolf with a WildMobs one on spawn (this can impact mod compatibility).");
    }
