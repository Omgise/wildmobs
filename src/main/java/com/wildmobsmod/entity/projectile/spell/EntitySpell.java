    ...
    public void onUpdate() {
        ...
        if (this.worldObj.isRemote && age > 0) {
            switch (this.getSpell()) {
                case 0:
                    ClientProxy.generateEntitySpellFX(
                        this,
                        this.posX,
                        this.posY,
                        this.posZ,
    ...
    protected void onImpact(MovingObjectPosition pos) {
        if (!this.worldObj.isRemote && pos.entityHit != null && pos.entityHit.riddenByEntity != this.getThrower()) {
            ...
            switch (this.getSpell()) {
                case 0:
                    pos.entityHit
                        .attackEntityFrom(DamageSource.causeIndirectMagicDamage(this, this.getThrower()), power + 2F);
                    break;
                case 1:
                    ((EntityLivingBase) pos.entityHit)
                        .addPotionEffect(new PotionEffect(WildMobsMod.aquaHealingID, power * 100, 0));
                    break;
            }
        }
    }
