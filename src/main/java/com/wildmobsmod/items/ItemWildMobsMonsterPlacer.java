...
public class ItemWildMobsMonsterPlacer extends ItemMonsterPlacer {
    ...
    public ItemWildMobsMonsterPlacer(String parEntityToSpawnName, int parPrimaryColor, int parSecondaryColor,
        int parEntityToSpawnVariant, boolean parBooleanType) {
    ...
    public ItemWildMobsMonsterPlacer(String parEntityToSpawnName, int parPrimaryColor, int parSecondaryColor,
        int parEntityToSpawnVariant, boolean parBooleanType, String parSpawnEggName) {
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        double yOffset = (side == 1 && world.getBlock(x, y, z)
            .getRenderType() == 11) ? 0.5D : 0.0D;
        ...
        if (entity != null) {
            ...
            player.addChatMessage(
                new ChatComponentText(EnumChatFormatting.RED + "Failed creating entity from spawnegg!"));
        }
        return true;
    }
    ...
    public String getItemStackDisplayName(ItemStack stack) {
        String baseName = StatCollector.translateToLocal(this.getUnlocalizedName() + ".name")
            .trim();
        if (!this.spawnEggName.isEmpty()) {
            return baseName + " "
                + StatCollector.translateToLocal("entity." + WildMobsMod.MODID + "." + spawnEggName + ".name");
        }
        if (!this.entityToSpawnName.isEmpty()) {
            return baseName + " "
                + StatCollector.translateToLocal("entity." + WildMobsMod.MODID + "." + entityToSpawnName + ".name");
        }
        return baseName;
    }
}
