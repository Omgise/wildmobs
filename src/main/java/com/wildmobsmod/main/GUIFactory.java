        ...
        public ModGuiConfig(GuiScreen guiScreen) {
            super(
                ...
                getAllConfigElements(
                    WildMobsMod.config,
                    WildMobsMod.CONFIG_CATEGORY_GENERAL,
                    WildMobsMod.CONFIG_CATEGORY_ENTITIES),
                WildMobsMod.MODID,
                true,
                true,
                GuiConfig.getAbridgedConfigPath(WildMobsMod.config.toString()));
        }
            ...
            for (String name : config.getCategoryNames()) {
                ...
                if (cat != null) {
                    if (inlines.contains(name)) {
                        elements.addAll(new ConfigElement(cat).getChildElements()); // getChildElements is *not*
                                                                                    // recursive!
                    } else if (!cat.isChild()) {
