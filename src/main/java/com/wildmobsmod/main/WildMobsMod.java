...
@Mod(
    modid = WildMobsMod.MODID,
    name = WildMobsMod.MODNAME,
    version = WildMobsMod.VERSION,
    guiFactory = "com.wildmobsmod.main.GUIFactory")
public class WildMobsMod {

    public static final String MODID = "wildmobsmod", MODNAME = "Wild Mobs", VERSION = Tags.VERSION;
    ...
    public static final boolean enableFoxUnnaturalVariants = true; // flag is still (technically) in use, but no longer
                                                                   // configurable (whether a functionality like this
                                                                   // should be available in a proper
                                                                   // SkinnedEntity-System remains to be decided)

    private static final String[] MIRED_LOOT = { "minecraft:bone, 1, 8", "minecraft:bone, 2, 4",
        "minecraft:wheat_seeds, 1, 7", "minecraft:wheat_seeds, 2, 2", "minecraft:arrow, 1, 5", "minecraft:arrow, 2, 2",
        "minecraft:rotten_flesh, 1, 6", "minecraft:stick, 1, 5", "minecraft:torch, 1, 4", "minecraft:bread, 1, 4",
        "minecraft:poisonous_potato, 1, 3", "minecraft:potato, 1, 2", "minecraft:coal:1, 1, 2", "minecraft:book, 1, 1",
        "minecraft:book, 2, 1", "minecraft:stone_sword, 1, 1", "minecraft:stone_pickaxe, 1, 1",
        "minecraft:stone_axe, 1, 1", "minecraft:stone_shovel, 1, 1", "minecraft:leather_helmet, 1, 1",
        "minecraft:leather_chestplate, 1, 1", "minecraft:leather_leggings, 1, 1", "minecraft:leather_boots, 1, 1",
        "minecraft:gold_nugget, 1, 1" };

    // Monster
    public static final EntityConfigDreathMired DREATH_MIRED_CONFIG = EntityConfigDreathMired
        .construct("Dreath", 5, 1, 1, true, true, true, 1.0D, true, 3, MIRED_LOOT, false);
    ...
    public static final EntityConfigJellyfish JELLYFISH_CONFIG = EntityConfigJellyfish
        .construct("Jellyfish", 9, 3, 4, true, 2);
    ...
    public void init(FMLInitializationEvent event) {
        ...
        // page or not.
        //
        // if(WildMobsMod.enableAchievementPage) // Achievements disabled for now - this should be redone and expanded
        // for the new Achievement system
        // {
        // achievementTameDireWolf = new Achievement("achievement.tameDireWolf", "tameDireWolf", 0, 0,
        // WildMobsModItems.thickBone, AchievementList.buildSword).initIndependentStat().registerStat();
        // achievementKillWizard = new Achievement("achievement.killWizard", "killWizard", 2, 0,
        // WildMobsModItems.magicBook, AchievementList.blazeRod).initIndependentStat().registerStat().setSpecial();
        // AchievementPage.registerAchievementPage(new AchievementPage("Wild Mobs", new Achievement[] {
        // WildMobsMod.achievementTameDireWolf, WildMobsMod.achievementKillWizard }));
        // }
        // else
        // {
        // achievementTameDireWolf = new Achievement("achievement.tameDireWolf", "tameDireWolf", 5, -4,
        // WildMobsModItems.thickBone, AchievementList.buildSword).initIndependentStat().registerStat();
        // achievementKillWizard = new Achievement("achievement.killWizard", "killWizard", 0, 12,
        // WildMobsModItems.magicBook, AchievementList.blazeRod).initIndependentStat().registerStat().setSpecial();
        // }
        ...
        potionAquaHealing = (new PotionAquaHealing(WildMobsMod.aquaHealingID)).setIconIndex(0, 0)
            .setPotionName("potion.wildmobs.aqua_healing");
    }
    ...
    public static void syncConfig() {
        skeletonWolfChance = config.getInt(
            "skeletonWolfChance",
            CONFIG_CATEGORY_ENTITIES,
            3,
            -1,
            100,
            "Chance (in Percent) for a Skeleton Wolf to spawn alongside a Skeleton (0 to disable; -1 to prevent the Skeleton Wolf Entity from being registered)");
        aquaHealingID = config.getInt(
            "aquaHealingId",
            CONFIG_CATEGORY_GENERAL,
            40,
            0,
            1024,
            "The Potion ID for Aqua Healing, please do your research before changing this!");
        enableCalamari = config
            .getBoolean("enableCalamari", CONFIG_CATEGORY_GENERAL, true, "Make Squids drop calamari");
    ...
    /**
     * <b>Required to prevent chanced replacement handlers from rolling every time the entity is loaded from a
     * save!</b><br>
     * Creates a Boolean NBTTag ({@link #ENTITY_LOADING_FLAG_NAME}) to track if an entity has been reviewed by this
     * function at any point.
     * Can be used to perform actions only when an entity is originally created and added to the world without that
     * action being repeated whenever that entity is loaded from the world save.<br>
     * Explicitly setting the Tag to false or deleting it will make the game "forget" the entity is being loaded and is
     * highly discouraged due to potential world-breakage.<br>
     * If possible this functionality should be replaced with Vanilla/Forge methods or at least a unified framework (if
     * multiple mods need to do this, they will otherwise all add their own Tags).<br>
     */
