...
public class OcelotOverrideHandler {
    ...
    public void onOcelotSpawn(EntityJoinWorldEvent event) {
        ...
        if (entity.getClass() == EntityOcelot.class && WildMobsMod.checkIsEntityNew(entity)
            && Math.random() * 100 < WildMobsMod.OCELOT_CONFIG.getOverrideChance()) {
            EntityOcelot ocelot = (EntityOcelot) entity;
            EntityWMOcelot newOcelot = new EntityWMOcelot(event.world);
            newOcelot
                .setLocationAndAngles(ocelot.posX, ocelot.posY, ocelot.posZ, ocelot.rotationYaw, ocelot.rotationPitch);
